...
<div class="section">
    ...
    {else}
    <div class="article">
    {if $follower_count_history_by_day.trend}<h3>
        {if $follower_count_history_by_day.trend > 0}
            <span style="color:green">+
        {else}<span style="color:red">
        {/if}
        {$follower_count_history_by_day.trend|number_format}</span>/day</h3>
    {/if}
...
<div class="section">
    ...
    {else}
    <div class="article">
    {if $follower_count_history_by_week.trend != 0}<h3>
        {if $follower_count_history_by_week.trend > 0}
            <span style="color:green">+
        {else}
            <span style="color:red">
        {/if}
        {$follower_count_history_by_week.trend|number_format}</span>/week</h3>
    {/if}
...
<div class="section">
    ...
    {else}
    <div class="article">
    {if $follower_count_history_by_month.trend != 0}
        <h3>
        {if $follower_count_history_by_month.trend > 0}
            <span style="color:green">+
        {else}
        ...
        {$follower_count_history_by_month.trend|number_format}</span>/month</h3>
    {/if}
...
<div class="section">
    ...
    {else}
    <div class="article">
    {if $list_membership_count_history_by_day.trend}
    <h3>
        {if $list_membership_count_history_by_day.trend > 0}
        <span style="color:green">+
        {else}
        <span style="color:red">
        {/if}
        
        {$list_membership_count_history_by_day.trend|number_format}</span>/day</h3>
    {/if}
    
    <img src="http://chart.apis.google.com/chart?chs=680x200&chxt=x,y&chxl=0:|{foreach from=$list_membership_count_history_by_day.history key=tid item=t name=foo}{$tid|date_format:"%b %d"}|{/foreach}1:|{foreach from=$list_membership_count_history_by_day.y_axis key=tid item=t name=foo}{$t|number_format}{if !$smarty.foreach.foo.last}|{/if}{/foreach}&cht=bvs&chco=FF9900&chd=t:{foreach from=$list_membership_count_history_by_day.history key=tid item=t name=foo}{if $t > 0}{$t}{else}_{/if}{if !$smarty.foreach.foo.last},{/if}{/foreach}&chbh=a&chds={$list_membership_count_history_by_day.min_count},{$list_membership_count_history_by_day.max_count}&chxr={$list_membership_count_history_by_day.min_count},{$list_membership_count_history_by_day.max_count}&chxs=1N*s*&chm=N*s*,E5B9D4,0,-1,10,,e::5" />
...
<div class="section">
    ...
    {else}
    <div class="article">
    {if $list_membership_count_history_by_week.trend != 0}
        <h3>
        {if $list_membership_count_history_by_week.trend > 0}<span style="color:green">+
        {else}<span style="color:red">
        {/if}
        
        {$list_membership_count_history_by_week.trend|number_format}</span>/week</h3>
    {/if}
    
    <img src="http://chart.apis.google.com/chart?chs=680x200&chxt=x,y&chxl=0:|{foreach from=$list_membership_count_history_by_week.history key=tid item=t name=foo}{$tid|date_format:"%b %d"}|{/foreach}1:|{foreach from=$list_membership_count_history_by_week.y_axis key=tid item=t name=foo}{$t|number_format}{if !$smarty.foreach.foo.last}|{/if}{/foreach}&cht=bvs&chco=FF9900&chd=t:{foreach from=$list_membership_count_history_by_week.history key=tid item=t name=foo}{if $t > 0}{$t}{else}_{/if}{if !$smarty.foreach.foo.last},{/if}{/foreach}&chbh=a&chds={$list_membership_count_history_by_week.min_count},{$list_membership_count_history_by_week.max_count}&chxr={$list_membership_count_history_by_week.min_count},{$list_membership_count_history_by_week.max_count}&chxs=1N*s*&chm=N*s*,E5B9D4,0,-1,10,,e::5" />
...
<div class="section">
    ...
    {else}
    <div class="article">
    {if $list_membership_count_history_by_month.trend != 0}
        <h3>
        {if $list_membership_count_history_by_month.trend > 0}<span style="color:green">+
        {else}<span style="color:red">
        {/if}
        
        {$list_membership_count_history_by_month.trend|number_format}</span>/month</h3>
    {/if}
    
    <img src="http://chart.apis.google.com/chart?chs=680x200&chxt=x,y&chxl=0:|{foreach from=$list_membership_count_history_by_month.history key=tid item=t name=foo}{$tid|date_format:"%b '%y"}|{/foreach}1:|{foreach from=$list_membership_count_history_by_month.y_axis key=tid item=t name=foo}{$t|number_format}{if !$smarty.foreach.foo.last}|{/if}{/foreach}&cht=bvs&chco=FF9900&chd=t:{foreach from=$list_membership_count_history_by_month.history key=tid item=t name=foo}{if $t > 0}{$t}{else}_{/if}{if !$smarty.foreach.foo.last},{/if}{/foreach}&chbh=a&chds={$list_membership_count_history_by_month.min_count},{$list_membership_count_history_by_month.max_count}&chxr={$list_membership_count_history_by_month.min_count},{$list_membership_count_history_by_month.max_count}&chxs=1N*s*&chm=N*s*,E5B9D4,0,-1,10,,e::5" />
